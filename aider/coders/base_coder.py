...
REPETITIVE_RESPONSE_MIN_SHORT_LINE_CHARS = 30
...
TEST_ERROR_REFLECTION_GUIDANCE = (
    ...
    "throws clauses, helper classes, coordinate origin/order, exact expected "
    "string literals, and behavior. Do not repeat the previous implementation "
    ...
    "context. When a failure shows an expected message, string, or rendered value, "
    "copy that expected value byte-for-byte, preserving digits, punctuation, "
    "capitalization, quote contents, and whitespace such as tabs/newlines/spaces; "
    "do not paraphrase words like zero/0 or rows/lines. If expected and actual "
    "differ only by whitespace, repair that whitespace directly. For parser "
    "escape failures, distinguish escaped newline folding from ordinary whitespace; "
    "preserve tabs and spaces unless the exercise rules say to normalize them. "
    "When a compiler reports a duplicate class, reuse the existing sibling class "
    "instead of declaring another one. When a compiler reports cannot find symbol "
    "on a helper, inspect and call the existing helper APIs, constructors, and "
    "method names instead of inventing replacements. When expected/actual values "
    "print as opaque object hashes, use nearby expected constructors and "
    "value-object helpers to infer fields. For Pair/Point/Position coordinates, "
    "convert internal zero-based row/column indexes to the one-based public "
    "coordinate system when expected constructors show one-based values. Match "
    "the failing assertion entry point, including constructor calls in lambdas. "
    "Return only corrected edits."
)
